...
from common import Todo, DayOfWeek
...
I wake up at 08:00 and finish my day at 21:30. Each habit must be scheduled exactly once. Sort the list by start time. Think step by step.
""".strip()
...
def create_schedule(habits_df: pd.DataFrame, day_of_week: DayOfWeek) -> List[Todo]:
    ...
    prompt = PromptTemplate(
        template=TODAY_HABITS_TEMPLATE,
        input_variables=["habits_table", "day_of_week"]
    )
    chain = LLMChain(
        llm=chat_gpt,
        prompt=prompt
    ...
    prompt = PromptTemplate(
        template=SCHEDULE_HABITS_TEMPLATE,
        input_variables=["habits_table"]
    )
    habits_table = scheduled_df[[
        "name", "preferred_time", "duration"]].to_markdown(index=None)
    chain = LLMChain(
        llm=chat_gpt,
        prompt=prompt
    )
...
def update_schedule(schedule: List[Todo], question: str) -> List[Todo]:
    ...
    prompt = PromptTemplate(
        template=SCHEDULE_QUESTION_TEMPLATE,
        input_variables=["schedule", "question"]
    )
